...
def remap_m6(self, **params):
    ...
    try:
        # Wait for the interpreter to be ready (only in MDI mode)
        if not is_auto_mode:
            timeout = 0
            while True:
                stat.poll()
