...
// Helper function to check if current tool is horizontal X-bit (T15 or T16)
function isHorizontalBit() {
  return tool && (tool.number == 15 || tool.number == 16);
}
...
function onRapid(_x, _y, _z) {
  ...
  // Swap X and Z for horizontal X-bits (T15/T16)
  if (isHorizontalBit()) {
    var temp = x;
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isHorizontalBit()) {
    ...
    // Swap start position for IJK calculations
    temp = start.x;
    start.x = start.z;
    start.z = temp;
  }
